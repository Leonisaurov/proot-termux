...
Los entrypoints auxiliares están agrupados por función: `proot/bin/appimage-run`
ejecuta AppImages y los wrappers Alpine viven en `proot/tests/rootfs/` para las
pruebas de PRoot.
...
comando), consulta [proot/docs/tools/proot-exec.md](proot/docs/tools/proot-exec.md) y copia
[proot/examples/proot-exec/proot-exec.conf.example](proot/examples/proot-exec/proot-exec.conf.example). El ejecutable es:

```bash
./proot/bin/proot-exec --config ./proot/examples/proot-exec/proot-exec.conf.example --dry-run
./proot/bin/proot-exec --config ./proot/examples/proot-exec/proot-exec.conf.example
```
...
4. Outputs a `.pkg.tar.xz` package in `proot/ci/termux/output/` in CI. Local builds
   write to `proot/artifacts/packages/` by default.

---

## Proot Source

The complete project lives under [`proot/`](./proot/). Its native PRoot source
is at [`proot/src/`](./proot/src/); the same tree also contains control-api,
...
1. Edit files under `proot/src/`
2. Bump `TERMUX_PKG_REVISION` in [`proot/ci/termux/packages/proot/build.sh`](./proot/ci/termux/packages/proot/build.sh)
3. Push — the [CI workflow](#cicd) triggers automatically

For local Termux builds use [`proot/scripts/build-native.sh`](./proot/scripts/build-native.sh),
not `make` directly. Its jobs option takes a separate argument (`-j 2` or
`--jobs 2`; `-j2` is invalid). The CI build copies `proot/src/` into the
...
Las regresiones nuevas se añaden como scripts revisables bajo `proot/tests/<tema>/`.
...
The launcher keeps normal Termux shebangs such as `#!/usr/proot/bin/env bash` and
`#!/proot/bin/bash` working in both launcher modes by binding the Termux executable
...
When started without a command, `proot/bin/termux-isolated` launches the shell selected
...
consumer such as [`proot/control-api`](./proot/control-api/) requires a compatible proot
...
The dependency is intentionally one-way: proot does not require `proot/control-api`,
...
|---|---|
| `libandroid-shmem` | [`proot/ci/termux/packages/libandroid-shmem/build.sh`](./proot/ci/termux/packages/libandroid-shmem/build.sh) |
| `libtalloc` | [`proot/ci/termux/packages/libtalloc/build.sh`](./proot/ci/termux/packages/libtalloc/build.sh) |

---

## CI/CD

The CI/package-builder checkout is kept under [`proot/ci/termux/`](./proot/ci/termux/).
...
[`proot/scripts/build-native.sh`](./proot/scripts/build-native.sh).
...
1. Edit source files under `proot/src/`
2. Bump `TERMUX_PKG_REVISION` in `proot/ci/termux/packages/proot/build.sh`
