...
class NetOperation(IntEnum): BIND=1; CONNECT=2; PUBLICATION=3; DNS=4
...
class ControlChannel:
    ...
    def _receive(self):
        ...
        if typ is Message.NET_ACCESS_REQUEST:
            ...
            if x[3] not in (2,10): self._fail(InvalidFrame('net family'))
    ...
    def _command(self,typ,payload,handler=None):
        ...
        while True:
            event=self.receive()
            if isinstance(event,CommandResult) and event.request_id==rid: return event
