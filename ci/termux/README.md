...
```bash
./ci/termux/scripts/run-docker.sh \
  ./ci/termux/build-package.sh -I -a aarch64 --format pacman proot
```

The container writes intermediate package output to `ci/termux/output/`.
The workflow collects the resulting archives into `artifacts/`; that directory
is intentionally separate from the local build output.

The proot package imports its source from `proot-source/src/` during
`termux_step_pre_configure()`. The package definition does not add Termux
bindings or harness policy to proot.

To clean the package-builder state, use `./ci/termux/clean.sh`. It is kept
...
Local native builds use `scripts/build-native.sh`, outside this directory. They
write packages to `artifacts/packages/` by default and support `-o DIR` for an
